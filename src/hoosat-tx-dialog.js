...
class HoosatTXDialog extends HoosatDialog {
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        .container {
          max-height: var(--hoosat-dialog-container-max-height, 90%);
          max-width: var(--hoosat-dialog-container-max-width, 90%);
        }
        .buttons {
  ...
  render() {
    ...
    return html`
      <div class="container">
        ...
        <div class="buttons">${this.renderButtons(args)}</div>
  ...
  renderHeading() {
    return html`${this.renderBackBtn()} ${T("Transactions")}
      <div class="flex"></div>
  ...
  renderBody(args) {
    let { skip } = this;
    let { items } = args;
    return html` ${this.wallet._renderAllTX({ skip, items })}
...
HoosatTXDialog.define("hoosat-tx-dialog");
