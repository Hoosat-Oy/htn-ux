...
import { html, css, HoosatDialog, askForPassword, HTN, formatForMachine, formatForHuman } from "./hoosat-dialog.js";

class HoosatQRScannerDialog extends HoosatDialog {
    ...
    return [
      ...
      css`
        .container {
          ...
          height: 100%;
          padding: 0px;
          max-height: var(--hoosat-dialog-container-max-height, 620px);
        }
        flow-t9 {
          width: 215px;
          margin: auto;
  ...
  renderBody() {
    ...
    return html`
      ...
      <div class="buttons">
        <flow-btn class="primary" @click="${this.sendBack}" i18n> Close </flow-btn>
      </div>
    `;
  }
  stopQRScanning() {
  ...
  async setValue(value) {
    ...
    if (value && this.isAddressQuery) {
      ...
      if (!isValid) this.setError(i18n.t("Invalid Address"));
    }
    this.isValid = isValid;
    if (isValid) this.sendValueBack();
  }
  open(args, callback) {
...
HoosatQRScannerDialog.define("hoosat-qrscanner-dialog");
