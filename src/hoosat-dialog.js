import { html, css, BaseElement, ScrollbarStyle, SpinnerStyle, UID } from "./flow-ux.js";
...
export class HoosatDialog extends BaseElement {
    ...
    return [
      ...
      css`
        :host {
          z-index: -10;
          opacity: 0;
          position: var(--hoosat-dialog-position, absolute);
          top: 0px;
          left: 0px;
          width: 100%;
          height: 100%;
          background-color: var(--hoosat-dialog-bg-color);
        ...
        .container {
          box-sizing: border-box;
          width: 100%;
          height: var(--hoosat-dialog-container-height, calc(100% - 10px));
          background-color: var(--flow-background-color, #f00);
          z-index: 1;
          border: var(--hoosat-dialog-container-border, 2px solid var(--flow-primary-color));
          border-radius: var(--hoosat-dialog-container-border-radius, 3px);
          max-width: var(--hoosat-dialog-container-max-width, 700px);
          max-height: var(--hoosat-dialog-container-max-height, 300px);
          margin: var(--hoosat-dialog-container-margin, 5px auto);
          padding: var(--hoosat-dialog-container-padding, 0px);
          position: relative;
          display: flex;
        ...
        :host([hideable]) .close-btn {
          display: var(--hoosat-dialog-container-close-btn-display, inline-block);
        }
        .heading {
          margin: 0px;
          padding: 5px 10px;
        ...
        .buttons {
          margin: var(--hoosat-dialog-buttons-margin, 10px auto);
          display: flex;
          width: var(--hoosat-dialog-buttons-width, 90%);
        }
        .buttons flow-btn {
          margin: 5px;
        }
  ...
  headingCls({ modeName }) {
    if (modeName == "Init") return "heading-init";
    return "";
  }
      ...
      <div class="container">
        ...
        <div class="flex body-box">
          <div class="body-inner">
            <div class="body">
              <div class="inner-body">${this.renderBody(args)}</div>
            </div>
            ${hasButtons ? html` <div class="buttons">${buttons}</div>` : ""}
            <span class="close-btn" title="Close" @click="${this.onCloseClick}">&times;</span>
          </div>
        </div>
      </div>
    `;
  ...
  attachUrlHistoryPopEvent() {
    if (this.withHistory) window.addEventListener("_popstate", this._onUrlHistoryPop);
  }
  ...
  renderBackBtn() {
    return html`<fa-icon class="back-btn" icon="arrow-alt-left" @click=${this.onBackClick}></fa-icon>`;
  }
  onBackClick() {
    this.hide();
  }
