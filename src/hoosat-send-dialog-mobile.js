import {
  ...
  formatForHuman,
  getLocalWallet,
} from "./hoosat-dialog.js";
import { Wallet } from "@hoosat/wallet-worker";
const pass = "";

class HoosatSendDialogMobile extends HoosatDialog {
  ...
  renderHeading({ estimating }) {
    return html`${this.renderBackBtn()} SEND
      <div class="flex"></div>
      ${estimating ? html`<fa-icon class="spinner" icon="sync"></fa-icon>` : ""}`;
  }
  renderBody({ estimating, estimateFee }) {
    return html`
      ...
      <div class="address-option-btns">
        <flow-btn @click="${this.scanQRCode}" class="primary" i18n>Scan QR Code</flow-btn>
        <flow-btn @click="${this.copyFromClipboard}" class="primary" i18n>Clipboard</flow-btn>
      ...
      >
        <flow-btn slot="suffix" class="primary" @click="${this.showT9}"><fa-icon icon="keyboard"></fa-icon></flow-btn>
      </flow-input>
      <flow-input
        class="fee full-width"
        suffix-btn
        outer-border
...
HoosatSendDialogMobile.define("hoosat-send-dialog-mobile");
