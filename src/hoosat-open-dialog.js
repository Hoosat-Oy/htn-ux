import { html, css, HoosatDialog, i18n, T } from "./hoosat-dialog.js";
import { askForPassword } from "./wallet.js";
const pass = "";

class HoosatOpenDialog extends HoosatDialog {
  ...
  static get styles() {
    return [
      ...
      css`
        .container {
          max-height: var(--hoosat-dialog-container-max-height, 600px);
        }
        :host([mode="create"]) .container {
          max-height: var(--hoosat-dialog-container-max-height, 500px);
        }
        :host([mode="init"]) .container {
          max-height: var(--hoosat-dialog-container-max-height, 200px);
        }
        :host([mode="recover"]) .container {
          max-height: var(--hoosat-dialog-container-max-height, 450px);
        }
        .buttons {
          justify-content: center;
          --hoosat-dialog-buttons-width: 100%;
        }
  ...
  renderInitUI() {
    return html` <div class="sub-heading text-center" is="i18n-div">Welcome to Hoosat Wallet</div> `;
  }
  renderRecoverUI() {
    let rows = [0, 1, 2];
    let cells = [0, 1, 2, 3];
  ...
  renderOpenUI() {
    ...
    return html`
      ...
      >
        <fa-icon class="input-type-btn" slot="sufix" @click="${this.changeInputType}" icon="${icon}"></fa-icon>
      </flow-input>
      <div class="error">${this.errorMessage}</div>
  ...
  _renderOpenButtons() {
    return html` <flow-btn @click="${(e) => (this.mode = "create")}" i18n>NEW WALLET</flow-btn>
  ...
  renderCreateButtons() {
    return html`
      ...
      <flow-btn ?hidden=${this.isFresh} @click="${(e) => (this.mode = "recover")}" i18n>I have a wallet</flow-btn>
      <flow-btn primary @click="${this.showSeeds}" i18n>Next</flow-btn>
    `;
  }
  renderInitButtons() {
    return html` <flow-btn class="primary" @click="${(e) => (this.mode = "create")}" i18n>Create New Wallet</flow-btn>
      <flow-btn class="primary" @click="${(e) => (this.mode = "recover")}" i18n>Recover from Seed</flow-btn>`;
  }
  renderRecoverButtons() {
    return html` <flow-btn @click="${this.cancelRecover}" i18n>Cancel</flow-btn>
  ...
  showSeeds() {
    ...
    if (password != password2) return this.setError(i18n.t("Passwords do not match"));

    this.callback(null, { mode: "create", password, dialog: this });
  }
  onSeedInput(e) {
    let input = e.target.closest("input.seed");
    if (!input || input.dataset.index != "0") return;
    let words = (input.value + "").trim().split(" ");
    if (words.length < 12) return;

    this.qSAll("input.seed.word").forEach((input) => {
      let index = input.dataset.index;
      if (words[index] == undefined) input.value = "";
      else input.value = words[index];
    });
  }
  recoverWallet() {
    let wordsMap = {};
    ...
    this.qSAll("input.seed.word").forEach((input) => {
      ...
      if (input.value.length < 2) isInvalid = true;
    });
    ...
    if (isInvalid || !words.join("").length) return this.setError(i18n.t("Please provide valid words"));
    ...
    askForPassword(
      ...
      ({ btn, password }) => {
        if (!password || btn != "confirm") return;
...
HoosatOpenDialog.define("hoosat-open-dialog");
