import {
  ...
  getLocalWallet,
  Wallet,
} from "./hoosat-wallet-ui.js";
export { isMobile, dontInitiatedComponent };
export class HoosatWalletMobile extends HoosatWalletUI {
    ...
    return [
      ...
      css`
        ...
        .header {
          ...
          align-items: center;
          min-height: 28px;
          padding: var(--hoosat-wallet-header-padding, 5px 10px);
          margin: var(--hoosat-wallet-header-margin, 0px);
        }

        .pagination a {
          border: var(
            --flow-btn-border,
            2px solid var(--flow-border-color, var(--flow-primary-color, rgba(0, 151, 115, 1)))
          );
        ...
        .pagination-box {
          padding: var(--hoosat-pagination-box-padding, 10px 5px;);
        }

        .logo {
          width: 30px;
          height: 30px; /*background-color:#DDD*/
          display: var(--hoosat-wallet-header-logo-diaplay, initial);
        }
        .logo-img {
          max-width: 100%;
          max-height: 100%;
        ...
        .tabs-container {
          overflow: hidden;
          overflow-x: auto;
          border-top: 2px solid var(--hoosat-wallet-tab-border-top-color, var(--flow-primary-color));
          border-bottom: 1px solid var(--hoosat-wallet-tab-border-bottom-color, #ddd);
        }
        .tabs {
          display: flex;
          align-items: stretch;
        ...
        .tab.selected {
          border-bottom-color: var(--hoosat-wallet-tab-active-border-color, var(--flow-primary-color));
        }
        .tab:not(.selected) {
          cursor: pointer;
        }
        ...
        .address-input {
          ...
          max-width: 500px;
          width: 100px;
          min-width: var(--hoosat-wallet-address-input-min-width, 460px);
        ...
        .send-scan-buttons a fa-icon {
          ...
          border-radius: 50%;
          padding: 22px;
          background-color: var(--hoosat-wallet-scan-button-bg, #3d4e58);
          --fa-icon-color: var(--hoosat-wallet-scan-button-color, #fff);
          box-shadow: var(--flow-box-shadow);
        }
        .send-scan-buttons .send-btn fa-icon {
          background-color: var(--hoosat-wallet-send-button-bg, #fb7470);
          --fa-icon-color: var(--hoosat-wallet-send-button-color, #fff);
        }
        .send-scan-buttons .receive-btn fa-icon {
          background-color: var(--hoosat-wallet-receive-button-bg, #60b686);
          --fa-icon-color: var(--hoosat-wallet-receive-button-color, #fff);
        }
        ...
        .lock-screen {
          ...
          position: absolute;
          z-index: 1000000;
          background-color: var(--hoosat-lock-screen-bg-color);
          top: 0px;
          bottom: 0px;
          left: 0px;
          right: 0px;
  ...
  toggleFullScreen() {
    if (this.fullscreen) document.webkitExitFullscreen();
  ...
  render() {
    ...
    return html`
      ...
      <div class="tabs-container hide-scrollbar" ?not-ready=${!isReady}>
        <flow-menu class="tabs" selected="${selectedTab}" selector=".tab" valueAttr="tab" @select="${this.onTabSelect}">
          ${this.showBalanceTab
            ? html`<a class="tab" tab="balance" href="javascript:void 0" is="i18n-a">Balance</a>`
            : ""}
          ...
          ${this.hideFaucet ? "" : html`<a class="tab" tab="faucet" href="javascript:void 0" is="i18n-a">Faucet</a>`}
          ${this.hideNetwork ? "" : html`<a class="tab" tab="network" href="javascript:void 0" is="i18n-a">Network</a>`}
          ${this.hideDebug ? "" : html`<a class="tab" tab="debuginfo" href="javascript:void 0" is="i18n-a">Debug</a>`}
          ${this.hideUTXOs
            ? ""
            : html`<a class="tab" tab="utxos" href="javascript:void 0" is="i18n-a">UTXOs</a>
                <fa-icon
                  ?hidden=${selectedTab != "utxos"}
                  class="reload-utxo-btn ${this.reloadingUTXOs ? "spinner" : ""}"
                  icon="sync"
                  @click=${this.reloadUTXOs}
                ></fa-icon>`}
        </flow-menu>
      </div>
      <div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
        <div class="flow-swipeable-row">
          ${this.showBalanceTab
            ? html`<div class="tab-content ${sCls("balance")}" for="balance">
                <div class="error-message" ?hidden=${!this.errorMessage}>${this.errorMessage}</div>
                ${this.renderAddressAndQr()} ${this.renderBalanceAndButton()}
                ${this.renderTX({ hideTxBtn: true, onlyNonConfirmed: true })}
              </div>`
            : ""}
          <div class="tab-content v-box pb-0 ${sCls("transactions")}" for="transactions">${this.renderAllTX()}</div>
          <div class="tab-content ${sCls("wallet")}" for="wallet">
            <div class="wallet-ux">
              ...
              <div class="badge">
                <span is="i18n-span">Network:</span> ${(this.receiveAddress || "").split(":")[0] || ""}
              </div>

              <flow-btn class="center-btn primary v-margin" @click="${this.compoundUTXOs}" i18n
                >Compound Transactions</flow-btn
              >
              <flow-btn class="center-btn primary v-margin" @click="${this.exportTransactions}" i18n
                >Export transactions as CSV</flow-btn
              >
              <flow-btn
                class="center-btn primary v-margin"
                ?disabled=${this.updatingTransactionsTime}
                @click="${this.updateTransactionsTime}"
                i18n
                >Update transaction times</flow-btn
              >
                ...
                ? html`
                    <div class="transactions-update-status">
                      ...
                      this.updatingTransactionsTimeStatus.total == this.updatingTransactionsTimeStatus.updated
                        ...
                        : html`Transactions updated: ${this.updatingTransactionsTimeStatus.updated} /
                          ${this.updatingTransactionsTimeStatus.total}`}
                    </div>
                  `
                : ""}
              <flow-btn class="center-btn primary v-margin" @click="${this.showSeeds}" i18n>Backup Seed</flow-btn>
              <flow-btn class="center-btn primary v-margin" @click="${this.showRecoverWallet}" i18n
                >Recover From Seed</flow-btn
              >
              <flow-btn class="center-btn primary v-margin" @click="${this.exportWalletFile}" i18n
                >Export Wallet Seed File</flow-btn
              >
              <flow-btn class="center-btn primary v-margin" @click="${this.importWalletFile}" i18n
                >Import Wallet Seed File</flow-btn
              >
              ...
              <flow-expandable class="donation-info" no-info no-icon icon="-">
                ...
                <p is="i18n-p">
                  if you wish to further the development of the hoosat ecosystem, we accept HTN donations at the
                  following addresses:
                </p>
                ${donationAddresses.map((t) => {
                  let [title, address] = t;
                  return html` <div class="donation-address-box badge">
                    <flow-i18n text="${title}"></flow-i18n>
                    <input class="address" value="${address}" />
                    <fa-icon
                      @click="${this.copyDonationAddress}"
                      icon="copy"
              ...
              <flow-expandable class="developer-info" _expand no-info no-icon icon="-">
                ...
                <div class="badge">
                  <span is="i18n-span">Hoosat Core:</span> ${window.PWA_MODULES["@hoosat/core-lib"]}
                </div>
                <div class="badge">
                  <span is="i18n-span">Hoosat Wallet Framework:</span> ${window.PWA_MODULES["@hoosat/wallet"]}
                </div>
                <div class="badge"><span is="i18n-span">Hoosat gRPC:</span> ${window.PWA_MODULES["@kaspa/grpc"]}</div>
                <div class="badge">
                  <span is="i18n-span">Hoosat gRPC Relay:</span> ${window.PWA_MODULES["@kaspa/grpc-web"]}
                </div>
                <div class="badge"><span is="i18n-span">Hoosat UX:</span> ${window.PWA_MODULES["@hoosat/ux"]}</div>
                <div class="badge">
                  <span is="i18n-span">Flow UX:</span> ${window.PWA_MODULES["@aspectron/flow-ux"]}
                </div>
              </flow-expandable>
            </div>
          </div>
          ${this.hideFaucet
            ? ""
            : html` <div class="tab-content ${sCls("faucet")}" for="faucet">
                ${this.faucetStatus
                  ? this.faucetStatus
                  : html`
                      <div class="faucet-ux">
                        ...
                        <div class="margin">
                          ${i18nHTMLFormat("You have <b>[n] HTN</b> available.", HTN(this.faucetFundsAvailable || 0))}
                        </div>
                        ...
                        ${this.faucetPeriod
                          ? html`
                              <div class="margin-bottom">
                                ${i18nHTMLFormat(
                                  "Additional funds will be<br/>available in [n]",
                                  FlowFormat.duration(this.faucetPeriod)
                                )}
                              </div>
                            `
                          : ``}
                        ${!this.faucetFundsAvailable
                          ? html``
                          : html`
                              <flow-btn class="primary" @click="${this.requestFaucetFunds}" i18n
                                >Request Funds from Faucet</flow-btn
                              >
                            `}
                      </div>
                    `}
          ...
          ${this.hideNetwork
            ? ""
            : html` <div class="tab-content ${sCls("network")}" for="network">
                        ...
                        <div>
                          <table>
                            ...
                            <tr>
                              ...
                              <td>
                                ${this.pastMedianTime
                                  ? new Date(this.pastMedianTime).toJSON().replace(/T/, " ").replace(/\..+$/, "")
                                  : ""}
                              </td>
                            </tr>
                          </table>
          ...
          ${this.hideDebug
            ? ""
            : html` <div class="tab-content ${sCls("debuginfo")}" for="debuginfo">
                <div class="info-ux">
                  <div class="caption">
                    <flow-i18n>IN USE UTXOS</flow-i18n>
                    <fa-icon
                      class="clear-used-utxos"
                      title="${T("Clear used UTXOs")}"
                      icon="broom"
                      @click="${this.clearUsedUTXOs}"
                    ></fa-icon>
                  </div>
                  <table>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td>${HTN(inUseUTXOs.satoshis || 0)} HTN</td>
                    </tr>
                  </table>
                  <flow-btn class="center-btn primary v-margin" @click="${this.showUTXOs}" i18n>Show UTXOs</flow-btn>
                  <flow-btn class="center-btn primary v-margin" @click="${this.forceTxTimeUpdate}" i18n
                    >Force transaction times update</flow-btn
                  >
                  <flow-btn class="center-btn primary v-margin" @click="${this.scanMoreAddresses}" i18n
                    >Scan More Addresses</flow-btn
                  >
          ...
          ${this.hideUTXOs
            ? ""
            : html`<div class="tab-content v-box pb-0 ${sCls("utxos")}" for="utxos">${this.renderUTXOs()}</div>`}
        </div>
      </div>
  ...
  renderHeaderBar() {
    ...
    return html`
      <div class="header" ?not-ready=${!isReady}>
        <div class="logo">
          <img class="logo-img" @click=${this.toggleFullScreen} src="/resources/logo512.png" />
        </div>
        <div class="flex"></div>
        ...
        <fa-icon ?hidden=${!loadingIndicator} class="spinner" icon="sync" style="position:absolute"></fa-icon>
        <fa-icon
          class="lock-btn"
          ?hidden=${this.locked || !this.wallet}
          @click=${this.lockWallet}
          icon="lock"
        ></fa-icon>
        ${this.hideI18nIcon
          ? ""
          : html`<div class="language-icon">
              <fa-icon icon="icons:language" @click="${this.onLangClick}"></fa-icon>
            </div>`}
        <fa-icon class="theme-btn" @click=${this.toggleTheme} icon="${theme == "light" ? "moon" : "sun"}"></fa-icon>
      </div>
    `;
  }
  ...
  unlockWallet() {
    askForPassword({ confirmBtnText: i18n.t("UNLOCK") }, async ({ btn, password }) => {
      ...
      if (!valid) return FlowDialog.alert(i18n.t("Error"), i18n.t("Invalid password"));
  ...
  renderMenu() {
    ...
    return html` <flow-dropdown class="icon-trigger top-menu right-align">
  ...
  renderAddressAndQr() {
    ...
    return html` <div class="address-and-qr">
  ...
  renderBalanceAndButton() {
    if (!this.wallet || !this.wallet.balance) return html``;
    ...
    return html`
      <div class="balance-badge">
        ${this.isLoading
          ? html`
              ...
              <div class="balance pending">
                <span class="label-pending"
                  >${T("PLEASE WAIT")} <span class="dots">${this.dots}</span> ${total
                    ? this.formatHTN(total) + " HTN"
                    : ""}</span
                >
              </div>
            `
          : html`
              <div class="balance">
                <span class="value">${this.formatHTN(available)} HTN</span>
              </div>
              <div class="balance pending">
                <span class="label-pending" is="i18n-span">Pending:</span>
                <span class="value-pending">${this.formatHTN(pending)} HTN</span>
              </div>
            `}
      </div>
