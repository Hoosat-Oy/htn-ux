...
import { initHoosatFramework, Wallet, workerLog } from "@hoosat/wallet-worker";
export { Wallet };
...
export class HoosatWalletUI extends BaseElement {
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        .recent-transactions .tx-title {
          width: 100%;
          display: flex;
          align-items: center;
          margin-bottom: 10px;
        }
        .recent-transactions .tx-row {
          position: relative;
        }
        .recent-transactions .tx-progressbar {
          position: absolute;
          left: 0px;
        }
        ...
        .recent-transactions [txout] .amount {
          color: #f00;
        }
  ...
  constructor() {
    ...
    this.throttledCompoundUTXO = throttle(async () => {
      if (this.waitingForCompound) return;
  ...
  async initNetworkSettings() {
    ...
    if (!this.rpcBuilder) return false;

    //const { network, port } = this.local_hoosatd_settings;
  ...
  async connectRPC() {
    if (this.rpc) return this.rpc.connect();
  }

  initDaemonRPC() {
    if (this.networkStatusUpdates || !window.flow?.app?.rpc?.subscribe) return;
    const { rpc } = flow.app;
    this.networkStatusUpdates = rpc.subscribe(`network-status`);
    (async () => {
      for await (const msg of this.networkStatusUpdates) {
        const { blockCount, headerCount, difficulty, networkName, pastMedianTime, pastMedianTimeDiff } = msg.data;
  ...
  initHelpers() {
    if (this._initHelpersInterval) return;
  ...
  renderTX({ hideTxBtn = false, onlyNonConfirmed = false } = {}) {
    ...
    if (onlyNonConfirmed) {
      if (blueScore) {
        items = this.txs.slice(0, 6).filter((tx) => {
          ...
          if (blueScore < bScore || !bScore) return false;
    ...
    if (hideTxBtn && !items.length && !this.preparingTxNotifications.size) return "";

    let color, p, cfmP, cfm;

    let notifications = [...this.preparingTxNotifications.values()];

    return html` <div class="recent-transactions">
      <div class="heading">${this.recentTransactionsHeading}</div>
      <div class="tx-notifications">
        ${notifications.map((n) => {
          return html`<div class="tx-notification">
            ${n.compoundUTXOs
              ? T(`Compounding UTXOs...`)
              : i18n.t(`Preparing transaction for [n] HTN ....`).replace("[n]", this.formatHTN(n.amount))}
          </div>`;
        })}
      </div>
      <div class="tx-rows">
        ${items.map((tx) => {
          ...
          p = cfmP / COUNT;
          if (p > 0.7) color = "#60b686";
          else if (p > 0.5) color = "orange";
          else color = "red";
          ...
          return html`
            <flow-expandable
              ...
              ?txmoved=${tx.isMoved}
              ?txout=${!tx.in}
              icon="${tx.in ? "sign-in" : "sign-out"}"
              no-info
            >
              <div class="tx-title" slot="title">
                <div class="tx-date flex">${tx.date}</div>
                <div class="amount">${tx.in ? "" : "-"}${this.formatHTN(tx.amount)} HTN</div>
              </div>
              ${(0 <= cfm) & (cfm <= COUNT)
                ? html`<flow-progressbar
                    class="tx-progressbar"
    ...
    return html`
      ...
      <div class="tx-list">
        ${items.map((tx, i) => {
          ...
          if (blueScore < bScore) cfm = COUNT + 1;
          cfmP = Math.min(COUNT, cfm);
          p = cfmP / COUNT;
          if (p > 0.7) color = "#60b686";
          else if (p > 0.5) color = "orange";
  ...
  async updateTransactionsTimeImpl(version) {
    ...
    if (this.updatingTransactionsTime) {
      FlowDialog.alert(i18n.t("Updating transaction times"), i18n.t("Transactions update process is in-progress"));
      return;
    }
    ...
    return new Promise((resolve) => {
      dpc(2000, async () => {
        ...
        let response = await this.wallet.startUpdatingTransactions(version).catch((err) => {
        ...
        if (response) console.log("updateTransactionsTime response", response);
        resolve();
      });
    });
  }
  ...
  renderAllTX() {
    ...
    return html`
      ${this._renderAllTX({ skip: txSkip, items })} ${renderPagination(pagination, this._onTXPaginationClick)}
    `;
  }
  _renderUTXOs({ skip, items }) {
    ...
    return html`
      ...
      <div class="tx-list">
        ${items.map((tx, i) => {
          return html` <div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
            <div class="tx-date" title="#${skip + i + 1} UTXO">${tx.blockDaaScore} (${tx.mass})</div>
  ...
  renderUTXOs() {
    ...
    return html` ${this._renderUTXOs({ skip, items })} ${renderPagination(pagination, this._onUTXOPaginationClick)}`;
  }
  ...
  importWalletFile() {
    ...
    input.onchange = (e) => {
      ...
      if (ext != "kpk") return invalidFileAlert();
      ...
      reader.onload = async (evt) => {
        ...
        try {
          ...
          if (!walletInfo?.wallet?.mnemonic) return invalidFileAlert();
  ...
  async showRecoverWallet() {
    let title = html`<fa-icon class="big warning" icon="exclamation-triangle"></fa-icon> ${T("Attention !")}`;
    let body = html`
  ...
  async setWallet(wallet) {
    if (localStorage.walletLogLevel) wallet.setLogLevel(localStorage.walletLogLevel);
  ...
  refreshStats() {
    ...
    if (this.blockCount == 1) {
      ...
      if (this.sync && this.sync < 99.95) status = i18n.t(`Syncing DAG [n]`).replace("[n]", `${this.sync.toFixed(2)}%`);
    }
    this.status = status;
  }
  ...
  async compoundIfNeeded(utxoCount) {
    if (utxoCount < MAX_UTXOS_THRESHOLD_COMPOUND) return;
  ...
  buildDebugInfoForDialog(err) {
    ...
    if (debugInfo || extraDebugInfo) {
      return html` <flow-expandable no-info>
  ...
  _compoundUTXOs(delay = 500, errorAlert = true) {
    ...
    return new Promise((resolve) => {
      dpc(delay, async () => {
        ...
        let response = await this.wallet.compoundUTXOs({ useLatestChangeAddress }).catch((err) => {
          ...
          if (errorAlert && !error.includes("Amount is expected")) {
            FlowDialog.alert(i18n.t("Error"), html`${error}${this.buildDebugInfoForDialog(err)}`);
          }
        });
        if (response) console.log("compoundUTXOs response", response);
  ...
  async scanMoreAddresses() {
    ...
    dpc(500, async () => {
      let response = await this.wallet.scanMoreAddresses(10000, false, receiveEnd, changeEnd).catch((err) => {
        ...
        if (typeof error == "string") FlowDialog.alert(i18n.t("Error"), error);
      });
      if (response) {
    ...
    return new Promise((resolve, reject) => {
      ...
      wallet.on("transactions-update-status", (info) => {
        ...
        if (info.total != undefined) {
          this.updatingTransactionsTimeStatus = Object.assign(this.updatingTransactionsTimeStatus || {}, info);
        }
      ...
      wallet.on("new-transaction", (tx) => {
        ...
        this.requestUpdate("balance", null);
        if (this.txDialog) this.txDialog.onNewTx(tx);
      });
  ...
  renderExtraScaning() {
    ...
    let { status, error, receiveStart, changeStart, receiveFinal, changeFinal, receiveProgress, changeProgress } =
      this.extraScanning;

    if (receiveProgress == receiveStart) receiveProgress = "";
    if (changeProgress == changeStart) changeProgress = "";
  ...
  async loadData() {
    let dots = setInterval(() => {
      this.dots += ".";
      if (this.dots.length > 5) this.dots = ".";
    }, 333);
    try {
      this.isLoading = true;
  ...
  connectedCallback() {
    ...
    let openDialog = document.createElement("hoosat-open-dialog");
    openDialog.hideLogo = !!this.hideOpenWalletLogo;
    this.parentNode.insertBefore(openDialog, this.nextSibling);
    this.sendDialog = document.createElement("hoosat-send-dialog" + mobileSuffix);
    this.parentNode.appendChild(this.sendDialog);
    this.receiveDialog = document.createElement("hoosat-receive-dialog" + mobileSuffix);
    this.parentNode.appendChild(this.receiveDialog);
    this.seedsDialog = document.createElement("hoosat-seeds-dialog");
    this.parentNode.appendChild(this.seedsDialog);
    let t9Dialog = document.createElement("hoosat-t9-dialog");
    this.parentNode.appendChild(t9Dialog);
    let qrscannerDialog = document.createElement("hoosat-qrscanner-dialog");
    qrscannerDialog.debug = this.debugscanner;
    this.parentNode.appendChild(qrscannerDialog);
    let uploadFileDialog = document.createElement("hoosat-upload-file-dialog");
    this.parentNode.appendChild(uploadFileDialog);

    const { workerCorePath } = window.HoosatConfig || {};
    initHoosatFramework({
      workerPath: workerCorePath || "/hoosat-wallet-worker/worker.js?ident=" + (window.HoosatConfig?.ident || ""),
    }).then(() => {
      let encryptedMnemonic = getLocalWallet()?.mnemonic;
  ...
  async handleInitDialogCallback({ dialog, password, seedPhrase, encryptedMnemonic }) {
    ...
    if (!rpc) return FlowDialog.alert(i18n.t("Error"), i18n.t("Hoosat Daemon config is missing."));
    ...
    if (mode == "open") {
      const wallet = await Wallet.import(password, encryptedMnemonic, { network, rpc }).catch((error) => {
  ...
  showTxDialog() {
    if (!this.txDialog) {
      this.txDialog = document.createElement("hoosat-tx-dialog");
  ...
  async isValidAddress(address) {
    ...
    if (window.mobileMode && prefix == "hoosattest") return true;
  ...
  async getMiningAddress() {
    ...
    if (this.receiveAddress) return Promise.resolve(this.receiveAddress);
  ...
  async sendTx(args) {
    const { address, amount, note, fee, calculateNetworkFee, inclusiveFee } = args;
    ...
    const response = await this.wallet
      .submitTransaction({
        toAddr: address,
        amount,
        fee,
      ...
      .catch((err) => {
        ...
        if (/Invalid Argument/.test(error)) error = i18n.t("Please provide correct address and amount");
  ...
  async estimateTx(args) {
    ...
    const data = await this.wallet
      ...
      .catch((err) => {
        ...
        if (/Invalid Argument/.test(error)) error = i18n.t("Please provide amount");
  ...
  makeFaucetRequest(subject, args) {
    if (!window.flow?.app?.rpc?.request) return Promise.reject("flow.app.rpc issue");
  ...
  async getHoosatFromFaucet(amount) {
    ...
    showT9(
      ...
      ({ value: amount, dialog }) => {
        ...
        if (sompis > this.faucetFundsAvailable) {
          let msg = i18n.t(`You can't request more than [n] HTN.`).replace("[n]", HTN(this.faucetFundsAvailable || 0));
          return dialog.setError(msg); //'
        }

        dialog.hide();

        this.getHoosatFromFaucet(amount);
      }
    );
  }
