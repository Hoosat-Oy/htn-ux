import { html, css, HoosatDialog, chunks, getRandomInt, shuffle, T, i18n } from "./hoosat-dialog.js";

class HoosatSeedsDialog extends HoosatDialog {
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        .container {
          max-height: var(--hoosat-dialog-container-max-height, 660px);
        }
        .buttons {
  ...
  renderStep1() {
    ...
    return html`
      <p is="i18n-p">
        Your wallet is accessible by a seed phrase. The seed phrase is an ordered 12-word secret phrase.
      </p>
      <p is="i18n-p">
        Make sure no one is looking, as anyone with your seed phrase can access your wallet your funds. Write it down
        and keep it safe.
      </p>
      <div class="words">
    ...
    return html`
      <div class="dots">
        ${this.indexes.map((v, index) => {
          ...
          if (index < this.varificationStep) icon = "check";
          else if (index > this.varificationStep) icon = "circle";
      ...
      <div class="button-row" @click="${this.wordClick}">
        ${words.map((word) => {
          return html` <flow-btn class="cell" data-word="${word}"> ${word} </flow-btn> `;
        })}
      </div>
    `;
  }
  ...
  renderStep1Buttons() {
    if (this.args?.showOnlySeed) return;
...
HoosatSeedsDialog.define("hoosat-seeds-dialog");
