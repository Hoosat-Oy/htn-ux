...
import { helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT } from "@hoosat/wallet-worker";
...
let { baseUrl, debug, MAX_UTXOS_THRESHOLD = 80, dontInitiatedComponent = false } = window.HoosatConfig || {};
if (!baseUrl) {
  baseUrl = new URL("../", import.meta.url).href;
  debug && console.log("HoosatUX: baseUrl", baseUrl);
}
...
/*
const {Wallet, initHoosatFramework, Storage} = require("hoosat-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("hoosat-grpc-node");
*/
...
export const getLocalWallet = () => {
  ...
  if (meta.wallet) meta.mnemonic = meta.wallet.mnemonic;
  return meta;
};

export const setLocalWallet = async (wallet, meta = {}) => {
  let oldWallet = await getLocalWallet();
  if (oldWallet) return storage.createWallet(wallet, meta);
  ...
  let body = () => {
    return html`
      ...
      <flow-input
        ...
        value="${pass}"
      >
        <fa-icon class="fa-btn" slot="sufix" @click="${changeInputType}" icon="${icon}"></fa-icon>
      </flow-input>
