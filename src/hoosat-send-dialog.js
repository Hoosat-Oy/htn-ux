...
import { html, css, HoosatDialog, askForPassword, HTN, formatForMachine, getLocalWallet } from "./hoosat-dialog.js";
const pass = "";
import { Wallet } from "@hoosat/wallet-worker";

class HoosatSendDialog extends HoosatDialog {
  ...
  static get styles() {
    return [
      ...
      css`
        .container {
          max-height: var(--hoosat-dialog-container-max-height, 670px);
        }
        .buttons {
  ...
  renderBody() {
    return html` <flow-input
        class="address full-width"
        outer-border
        label="${T(`Recipient Address (Must start with 'hoosat' prefix)`)}"
        value="${this.address || ""}"
        @changed="${this.onAddressChange}"
        placeholder="hoosat:recipient_address"
      >
      </flow-input>
      <div col>
        <flow-input
          class="amount full-width"
          outer-border
          label="${T("Amount in HTN")}"
          @keyup=${this.onAmountChange}
          value="${this.amount}"
        >
        </flow-input>
        <div spacer></div>
        <flow-input class="fee full-width" label="${T("Priority Fee")}" @keyup="${this.onNetworkFeeChange}">
        </flow-input>
      </div>
      <flow-input class="note full-width" outer-border label="${T("Note")}"></flow-input>
      <flow-checkbox class="calculate-network-fee" checked @changed="${this.onCalculateFeeChange}"
        >${T("Automatically calculate network fee")}</flow-checkbox
      >
      <flow-checkbox class="inclusive-fee" @changed="${this.onInclusiveFeeChange}"
        >${T("Include fee in the amount")}</flow-checkbox
      >
      ${this.renderEstimate()}
      <div class="error">${this.errorMessage}</div>`;
  }
  renderEstimate() {
    if (this.estimateError) return html`<div class="estimate-tx-error">${this.estimateError}</div>`;
  ...
  renderButtons() {
    ...
    return html` ${estimating ? html`<fa-icon class="spinner" icon="sync" style__="position:absolute"></fa-icon>` : ""}
      <flow-btn @click="${this.cancel}" i18n>Cancel</flow-btn>
      <flow-btn secondary class="burn" ?disabled=${this.amount === ""} @click="${this.burnAfterConfirming}" i18n
        >BURN
      </flow-btn>
      <flow-btn
        primary
        ?disabled=${estimating || !this.estimateTxSignal || !estimateFee}
        @click="${this.sendAfterConfirming}"
        i18n
        >SEND
      </flow-btn>`;
  }
  ...
  async burnAfterConfirming() {
    ...
    if (estimate.fee > this.alertFeeAmount) {
      ...
      let { btn } = await FlowDialog.alert("Warning", html`${msg}`, "", [
        {
          text: i18n.t("Cancel"),
          value: "cancel",
        },
...
HoosatSendDialog.define("hoosat-send-dialog");
